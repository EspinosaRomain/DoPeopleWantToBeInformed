...
use  "Data/Data_InfoABD.dta"

//Table for demographics
//Table C1: column 4
mat Demographics=J(15,1,.)
su stud_num if studentstatus=="Yes"
mat Demographics[1,1]=round(`r(N)'/201,0.001)*100
su stud_num if studentstatus=="No"
mat Demographics[2,1]=round(`r(N)'/201,0.001)*100
su stud_num if studentstatus=="Missing"
mat Demographics[3,1]=round(`r(N)'/201,0.001)*100
su sex_num if sex=="Male"
mat Demographics[4,1]=round(`r(N)'/201,0.001)*100
su sex_num if sex=="Female"
mat Demographics[5,1]=round(`r(N)'/201,0.001)*100
su sex_num if sex=="Missing"
mat Demographics[6,1]=round(`r(N)'/201,0.001)*100
su age
...
mat Demographics[9,1]=round(`r(N)'/201,0.001)*100
su employment_num if employmentstatus=="Full-Time"
mat Demographics[10,1]=round(`r(N)'/201,0.001)*100
su employment_num if employmentstatus=="Not in paid work (e.g. homemaker, retired or disabled)"
mat Demographics[11,1]=round(`r(N)'/201,0.001)*100
su employment_num if employmentstatus=="Other"
mat Demographics[12,1]=round(`r(N)'/201,0.001)*100
su employment_num if employmentstatus=="Part-Time"
mat Demographics[13,1]=round(`r(N)'/201,0.001)*100
su employment_num if employmentstatus=="Unemployed (and job seeking)"
mat Demographics[14,1]=round(`r(N)'/201,0.001)*100
su employment_num if employmentstatus=="Missing"
mat Demographics[15,1]=round(`r(N)'/201,0.001)*100
mat list Demographics

//Append data from main experiment
append using  "Data//Data_ABD.dta", force

keep if treatment=="vegan" | treatment=="hypenolink"
replace diet=class if diet==.
ranksum diet, by(treatment)
